...
user.prototype.produtosNoCarrinho = function(usuarioId, callback){
  this._connection.query('SELECT p.nome, p.preco, p.loja, p.desconto, p.galeria, c.quantidade, p.madeira, c.pecas, c.id FROM produtos p JOIN carrinho c ON p.id = c.produto_ID WHERE c.usuario_ID = ?', [usuarioId], callback)
}
